global  drSteps h_r m_r Ir_r h_s m_s Is_s h_p R_p beta_p chord_p H_p h_d R_d1 R_d2 beta_d chord_d H_d rho g S_s_f S_r_f area_p area_d bladeProperties_p bladeProperties_d1 bladeProperties_d2 m I_tot counter r_m K_t_m L_m rotorErrorAngle

disp('Setting parameters in setup_flyer_V1_5_prop_on_bottom');
    %Changable Parameters
        %Rotor properties
        h_r = .068; %Height of rotor cg below flyer cg (cg assumed axial) (meters) (positive is down)
        m_r = .063; %rotor mass in kg
%         Ir_r = [203536,0,0;0,26959,0;0,0,182477]*1e-9; %Rotor inertia at rotor cg in flyer frame
        Ir_r = [(203536+26959)/2,0,0;0,(203536+26959)/2,0;0,0,182477]*1e-9; %Rotor inertia at rotor cg in flyer frame

        %Stator properties
        h_s = -.03753; %Height of stator cg below flyer cg (cg assumed axial) (meters) (positive is down)
        m_s = .173; %stator mass in kg
        Is_s = [620000,0,0;0,620000,0;0,0,66438]*1e-9; %Stator inertia at stator cg in flyer frame
        ...
        h_d = h_p-.090;%-.08892; %Height of center of drag plate below stator cg (meters) (positive is down)
        ...
        beta_d = interp1(1:length(beta_d),beta_d',1:.1:length(beta_d)) + deg2rad(0);
        chord_d =[.105 .105 .099 .094 .086 .077 .065 .05 .03]; %wright bat drag plate
%         chord_d = [.152 .152 .152 .152 .152 .152 .152 .152 .152]; %blue drag plate
